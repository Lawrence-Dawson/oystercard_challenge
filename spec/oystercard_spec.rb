...
describe "Tests on topped up card" do
  ...
  it "deducts money from the card upon touch out" do
    expect{t_card.touch_out(a_station)}.to change{t_card.balance}.by(-1)
  end

  it "remembers entry station" do
    t_card.touch_in(a_station)
    expect(t_card.station_history[-1]).to include(:in => a_station)
  end

  # xit "entry station gets set to nill upon touch out" do
  #   t_card.touch_in(a_station)
  #   t_card.touch_out(a_station)
  #   expect(t_card.station_history).to eq nil
  # end

  it "stores an exit station for a journey" do
    t_card.touch_in(a_station)
    t_card.touch_out(a_station)
    expect(t_card.station_history[-1]).to include(:in => a_station, :out => a_station)
  end
